...
final class PhotosRepositoryTest: XCTestCase {
    ...
    func test_fetchPhotos_sendsUnsplashHeaders() async throws {
        MockURLProtocol.requestHandler = { request in
            (HTTPResponseFactory.make(url: request.url, statusCode: 200),
             JSONFixtures.photosArray(ids: ["a"]))
        }
        ...
        let headers = try XCTUnwrap(MockURLProtocol.lastRequest?.allHTTPHeaderFields)
        XCTAssertEqual(headers["Accept-Version"], "v1")
        XCTAssertEqual(headers["Authorization"], "Client-ID \(sut.clientId)")
    }
